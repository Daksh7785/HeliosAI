...
# Docs 35 & 36: Authentication & Authorization
SECRET_KEY = "super-secret-heliosai-key-replace-in-prod"
ALGORITHM = "HS256"
...
def require_role(required_role: str):
    ...
    async def role_checker(current_user: dict = Depends(get_current_user)):
        ...
        if roles_hierarchy.get(user_role, 0) < roles_hierarchy.get(required_role, 0):
            logger.warning(f"Access denied: {current_user['username']} lacks {required_role} role.")
            raise HTTPException(
